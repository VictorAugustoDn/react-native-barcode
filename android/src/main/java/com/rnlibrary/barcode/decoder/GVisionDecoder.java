...
public class GVisionDecoder extends ReactActivity implements Decoder {
    private final ImageScanner mScanner;
    private final ReactApplicationContext reactContext;

    public GVisionDecoder(ReactApplicationContext reactContext) {
    ...
    public ReadableMap decodeRGBBitmap(Bitmap bitmap) {
        ...
        try {
            int barcode = 0;
            BarcodeDetector detector = new BarcodeDetector.Builder(reactContext.getApplicationContext()).setBarcodeFormats(Barcode.ITF).build();
            Frame frame = new Frame.Builder().setBitmap(bitmap).build();
            SparseArray<Barcode> barcodes = detector.detect(frame);
            Barcode thisCode = barcodes.valueAt(0);
            barcode = Integer.parseInt(thisCode.rawValue);
            if (barcode != 0) {
                result = Arguments.createMap();
                result.putString("format", "ITF");
                result.putInt("content", barcode);
            }
        } catch (Exception e) {
        }
        return result;
